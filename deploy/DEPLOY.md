...
```bash
sudo cp deploy/chat-monitor-web.service deploy/chat-monitor-qq.service deploy/chat-monitor-dingtalk.service /etc/systemd/system/
sudo systemctl daemon-reload
sudo systemctl enable --now chat-monitor-web chat-monitor-qq chat-monitor-dingtalk
sudo systemctl status chat-monitor-web   # 查看状态
```

- 三个服务**互相独立**：一个崩溃/重启不影响其他；
...
- 登录后：总览页显示 QQ / 钉钉 平台状态（`运行中` 表示 worker 正常）；
...
| 重启 QQ worker | `sudo systemctl restart chat-monitor-qq` |
| 修改配置 | 改 `configs/*.env` 后 `sudo systemctl restart chat-monitor-qq chat-monitor-dingtalk chat-monitor-web` |
| 备份数据 | `sudo tar -czf backup.tar.gz /opt/chat-monitor/data /opt/chat-monitor/configs /opt/chat-monitor/reports` |
| 更新代码 | 重新上传 + `sudo systemctl restart chat-monitor-web chat-monitor-qq chat-monitor-dingtalk` |
...
| 6 | 密钥保护 | QQ/钉钉/AI Key 只存服务器，勿提交公开仓库；`configs/` 权限收紧 `chmod 600` |
